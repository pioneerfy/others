<template lang="html">
  <div :val="value_">
    <a-radio-group v-model="type">
      <div>
        <a-radio :value="1" border>每日</a-radio>
      </div>
      <div>
        <a-radio :value="2" border>不指定</a-radio>
      </div>
      <div>
        <a-radio :value="3" border>周期</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <a-input-number @change="type = 2" v-model="cycle.start" :min="1" :max="31" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">至</span>
        <a-input-number @change="type = 2" v-model="cycle.end" :min="2" :max="31" style="width: 100px;"></a-input-number>
        日
      </div>
      <div>
        <a-radio :value="3" border>循环</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <a-input-number @change="type = 3" v-model="loop.start" :min="1" :max="31" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">日开始，每</span>
        <a-input-number @change="type = 3" v-model="loop.end" :min="1" :max="31" style="width: 100px;"></a-input-number>
        日执行一次
      </div>
      <div>
        <a-radio :value="8" border>工作日</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">本月</span>
        <a-input-number @change="type = 8" v-model="work" :min="1" :max="7" style="width: 100px;"></a-input-number>
        号，最近的工作日
      </div>
      <div>
      ...
      <div>
        ...
        <a-checkbox-group v-model="appoint">
          <div  v-for="i in 4" :key="i" style="margin-left: 10px;  line-height: 25px;">
            <a-checkbox @change="type = 4"  v-for="j in 10" v-if="parseInt((i - 1) + '' + (j - 1)) < 32 && !(i === 1 && j === 1)" :key="j" :value="(i - 1) + '' + (j - 1)">{{ (i - 1) + '' + (j - 1) }}</a-checkbox>
          </div>
        </a-checkbox-group>
      </div>
    </a-radio-group>
