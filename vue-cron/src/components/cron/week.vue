<template lang="html">
  <div :val="value_">
    <a-radio-group v-model="type">
      <div>
        <a-radio :value="1" border>每周</a-radio>
      </div>
      <div>
      ...
      <div>
        ...
        <a-input-number @change="type = 2" v-model="cycle.start" :min="1" :max="7" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">至星期</span>
        <a-input-number @change="type = 2" v-model="cycle.end" :min="2" :max="7" style="width: 100px;"></a-input-number>
      </div>
      <div>
        <a-radio :value="3" border>循环</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">从星期</span>
        <a-input-number @change="type = 3" v-model="loop.start" :min="1" :max="7" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">开始，每</span>
        <a-input-number @change="type = 3" v-model="loop.end" :min="1" :max="7" style="width: 100px;"></a-input-number>
        天执行一次
      </div>
      <div>
        <a-radio :value="7" border>指定周</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">本月第</span>
        <a-input-number @change="type = 7" v-model="week.start" :min="1" :max="4" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">周，星期</span>
        <a-input-number @change="type = 7" v-model="week.end" :min="1" :max="7" style="width: 100px;"></a-input-number>
      </div>
      <div>
        <a-radio :value="6" border>本月最后一个</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">星期</span>
        <a-input-number @change="type = 6" v-model="last" :min="1" :max="7" style="width: 100px;"></a-input-number>
      </div>
      <div>
        <a-radio :value="4" border>指定</a-radio>
        <a-checkbox-group v-model="appoint" style="margin-left: 50px;  line-height: 25px;">
          <a-checkbox @change="type = 4"  v-for="i in 7" :key="i" :value="i">{{ i }}</a-checkbox>
        </a-checkbox-group>
      </div>
    </a-radio-group>
  </div>
