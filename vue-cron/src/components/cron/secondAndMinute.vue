...
<template lang="html">
  <div :val="value_">
    <a-radio-group v-model="type">
      <div>
        <a-radio :value="1" border>每{{ lable }}</a-radio>
      </div>
      <div>
        <a-radio :value="2" border>周期</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <a-input-number @change="type = 2" v-model="cycle.start" :min="1" :max="59" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">至</span>
        <a-input-number @change="type = 2" v-model="cycle.end" :min="2" :max="59" style="width: 100px;"></a-input-number>
        {{ lable }}
      </div>
      <div>
        <a-radio :value="3" border>循环</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <a-input-number @change="type = 3" v-model="loop.start" :min="0" :max="59" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">{{ lable }}开始，每</span>
        <a-input-number @change="type = 3" v-model="loop.end" :min="1" :max="59" style="width: 100px;"></a-input-number>
        {{ lable }}执行一次
      </div>
      <div>
        ...
        <a-checkbox-group v-model="appoint">
          <div  v-for="i in 6" :key="i" style="margin-left: 10px; line-height: 25px;">
            <a-checkbox @change="type = 4"  v-for="j in 10" :key="j" :value="(i - 1) + '' + (j - 1)">{{ (i - 1) + '' + (j - 1) }}</a-checkbox>
          </div>
        </a-checkbox-group>
      </div>
    </a-radio-group>
