<template lang="html">
  <div :val="value_">
    <a-radio-group v-model="type">
      <div>
        <a-radio :value="1" border>每月</a-radio>
      </div>
      <div>
      ...
      <div>
        ...
        <a-input-number @change="type = '2'" v-model="cycle.start" :min="1" :max="12" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">至</span>
        <a-input-number @change="type = '2'" v-model="cycle.end" :min="2" :max="12" style="width: 100px;"></a-input-number>
        月
      </div>
      <div>
        <a-radio :value="3" border>循环</a-radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <a-input-number @change="type = '3'" v-model="loop.start" :min="1" :max="12" style="width: 100px;"></a-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">月开始，每</span>
        <a-input-number @change="type = '3'" v-model="loop.end" :min="1" :max="12" style="width: 100px;"></a-input-number>
        月执行一次
      </div>
      <div>
        <a-radio :value="4" border>指定</a-radio>
        <a-checkbox-group v-model="appoint" style="margin-left: 0px;  line-height: 25px;">
          <a-checkbox @change="type = '4'"  v-for="i in 12" :key="i" :value="i">{{ i }}</a-checkbox>
        </a-checkbox-group>
      </div>
    </a-radio-group>
  </div>
